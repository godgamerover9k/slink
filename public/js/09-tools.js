import { ON } from "./01-engine.js";
import { satCount } from "./03-sat.js";
import { IDLE_MS, PENS, engine, ensureCells, indexTimer, me, now, pollTimer, queueCell, queueDiag, queueOp, room, setRoom, trial } from "./05-room-state.js";
import { badEls, cellSatisfied, dimClues, render, setDimClues, setMyName, setMyPen, setWeighted, weighted } from "./06-board.js";
import { doRedo, doUndo, keyBinds, resetView, setKeyBind, view, zoomAt } from "./07-input.js";
import { boardSnapshot, chainBranches, clearBranches, ensureTree, isOwner, ownerLabel, setChainBranches, setShowPremises, showPremises, switchBranch, trunk } from "./08-branches.js";
import { openSetup, roomLink } from "./10-setup.js";
...
var loadedFrom = null; // {name, handle} of the file this sheet came from
...
queueMicrotask(() => {
  ...
  document.getElementById("optDim").onchange = edge => {
    setDimClues(edge.target.checked);
    render();
  };
  document.getElementById("optPremise").onchange = edge => {
    setShowPremises(edge.target.checked);
    render();
  };
  document.getElementById("optWeight").onchange = ev => {
    setWeighted(ev.target.checked);
  ...
  (function wireChain() {
    ...
    if (!box) return;
    try {
      setChainBranches(window.localStorage.getItem("sl:chain") === "1");
    } catch (e) {}
    box.checked = chainBranches;
    box.onchange = () => {
      setChainBranches(box.checked);
      try {
