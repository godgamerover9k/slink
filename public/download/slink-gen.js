  ...
  for(;;){
    for(let i=0;i<stopAt;i++){
      ...
      if(onTick&&(++checked&15)===0){
        const info={stage:"trim",checked,pass,pct:Math.round(((i+1)/stopAt)*100),
                    left:[...clues].filter(v=>v>=0).length};
        /* The grid itself, less often: it is the whole point of watching, but
           it is one character per square every time. Always send the first,
           or a small puzzle finishes before the interval comes round. */
        if(checked===16||(checked&63)===0)
          info.grid={R,C,cells:[...clues].map(v=>v<0?".":String(v)).join("")};
        onTick(info);
      }
    }
    ...
    for(let i=0;i<nWorkers;i++){
      ...
      w.on("message",m=>{
        ...
        if(m.type==="tick"){
          ticks++;
          if(m.info){
            const had=byWorker.get(i);
            // the grid comes every so often; the rest comes constantly
            if(!m.info.grid&&had&&had.grid)m.info.grid=had.grid;
            byWorker.set(i,m.info);
          }
...
.watch{display:flex;align-items:center;gap:7px;font-size:12px;color:var(--muted);
  margin:0 0 10px;cursor:pointer}
.grid{font-family:ui-monospace,Consolas,monospace;font-size:9px;line-height:1.15;
  white-space:pre;color:var(--ink);background:var(--sheet);border:1px solid var(--rule);
  border-radius:3px;padding:6px;margin-top:6px;overflow:auto;max-height:220px}
.grid b{color:var(--accent);font-weight:600}
.each{margin:10px 0}
.each__top{display:flex;justify-content:space-between;font-family:ui-monospace,Consolas,monospace;
  font-size:10px;color:var(--muted)}
.each__note{font-family:ui-monospace,Consolas,monospace;font-size:10px;color:var(--muted);
  margin-top:2px}
.bar--thin{height:3px;margin:3px 0 0}
...
<div class="card">
  ...
  <label class="watch"><input type="checkbox" id="watch"> Watch it work</label>
...
/* The clue grid as it stands: dots where a clue has been taken out, so you can
   watch the puzzle being thinned. */
function drawGrid(g){
  const out=[];
  for(let r=0;r<g.R;r++){
    const row=g.cells.slice(r*g.C,(r+1)*g.C).split('');
    out.push(row.map(function(ch){ return ch==='.'?'<span style="opacity:.25">·</span>':'<b>'+ch+'</b>'; }).join(' '));
  }
  return out.join('\n');
}
let lastReady=false;      // something built and not yet saved
document.getElementById('stop').onclick=async()=>{
  document.getElementById('stop').disabled=true;
  await fetch('/stop',{method:'POST'});
};
document.getElementById('watch').onchange=function(e){ showGrid=e.target.checked; };

$('go').onclick=async()=>{
  if(lastReady&&!confirm(
      'The puzzles from the last run have not been saved. Starting again throws them away.'))
    return;
  ...
  poller=setInterval(async()=>{
    ...
    const perBox=$('per');
    if(perBox){
      if(s.done||!(s.each||[]).length)perBox.innerHTML='';
      else perBox.innerHTML=(s.each||[]).map(function(w){
        return '<div class="each"><div class="each__top"><span>puzzle '+(w.n+1)+'</span>'+
          '<span>'+(w.stage==='trim'?w.pct+'%':'')+'</span></div>'+
          '<div class="bar bar--thin"><div class="fill" style="width:'+w.pct+'%"></div></div>'+
          '<div class="each__note">'+w.note+'</div>'+
        (showGrid&&w.grid?'<div class="grid">'+drawGrid(w.grid)+'</div>':'')+
        '</div>';
      }).join('');
    }
...
function serveUI(opt){
  ...
  const server=http.createServer((req,res)=>{
    ...
    if(url==="/status"){
      ...
      const each=(pr.workers||[]).map(w=>({
        n:w.n,
        grid:w.grid||null,
        stage:w.stage||"",
        pct:w.stage==="trim"?(w.pct||0):0,
        note:w.stage==="loop"?`laying out a loop · attempt ${w.attempt}`
            :w.stage==="trim"?`trimming · ${w.left} clues left`
            :"starting",
      }));
