...
/* The page is no longer one file: it comes with a stylesheet, a folder of
   scripts and the downloadable generator, so whatever directory the page is
   found in is served as the web root. */
const TYPES = {
...
function makeServer(opt) {
  ...
  const server = http.createServer((req, res) => {
    ...
    // anything else under the page's folder: stylesheet, scripts, downloads
    if (u !== "/" && !u.startsWith("/kv/")) {
      const page = findPage();
      if (page) {
        const root = path.dirname(page);
        // resolve inside the root, so "../" cannot climb out of it
        const asked = path.normalize(path.join(root, decodeURIComponent(u)));
        if (asked.startsWith(root) && fs.existsSync(asked) && fs.statSync(asked).isFile()) {
          const type = TYPES[path.extname(asked).toLowerCase()] || "application/octet-stream";
          res.writeHead(200, { "Content-Type": type, "Cache-Control": "no-store" });
          return res.end(fs.readFileSync(asked));
        }
      }
    }
    if (u === "/" || u === "/index.html") {
