...
const PORT = 8177;
const srv = spawn('node', ['slink-server.js', '--port', String(PORT),
  '--page', pagePath(), '--data', '/tmp/rooms-std.json', '--noopen', '--open']);
let out = '';
...
(async () => {
  try {
    ...
    await wait(900);
    const saved = JSON.parse(fs.readFileSync('/tmp/rooms-std.json', 'utf8'));
