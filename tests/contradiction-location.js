...
(async()=>{
 ...
 console.log('\n--- and in the list, without selecting it ---');
 ev('switchBranch(null)'); ev('render()');
 const brokenRow=[...window.document.querySelectorAll('.tw')]
   .find(r=>/BROKEN/.test(r.textContent));
 ck('the broken branch is marked', !!brokenRow, true);
 const why=brokenRow && brokenRow.querySelector('.tw__why');
 ck('with the reason under it', !!why, true);
 console.log('   row said:', why ? why.textContent.slice(0,60) : '');
 ck('naming the square', /r\d+c\d+/.test(why ? why.textContent : ''), true);
 ck('while you stay on the master', ev('!!trial'), false);
 ck('a branch that is fine says nothing extra',
    ev(`[...document.querySelectorAll('.tw')].filter(r=>r.querySelector('.tw__why')).length`), 1);
