...
const PORT=8292, KEY='purple-otter-42';
const base=`http://127.0.0.1:${PORT}/`;
const srv=spawn('npm',['start'],{cwd:'onezip',env:{...process.env,PORT:String(PORT),SLINK_OPEN:'1'}});
...
(async()=>{
 try{
  ...
  ck('open to anyone, no key', /NO KEY/.test(out), true);
  ...
  const onServer=await fetch(base+'kv/sl:last');
